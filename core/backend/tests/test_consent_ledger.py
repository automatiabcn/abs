...
def test_set_and_channel_allow() -> None:
    ...
    assert (
        check_channel(
            tenant_slug="tC", contact_email="musteri@x.io", channel="whatsapp"
        )["allowed"]
        is False
    )
...
def test_do_not_call_blocks_phone() -> None:
    set_consent(
        tenant_slug="tC", contact_email="dnc@x.io", phone_consent=True, do_not_call=True
    )
...
def test_unknown_channel() -> None:
    assert (
        check_channel(
            tenant_slug="tC", contact_email="x@x.io", channel="carrierpigeon"
        )["allowed"]
        is False
    )
...
def test_tenant_isolation() -> None:
    ...
    assert (
        check_channel(tenant_slug="tCb", contact_email="shared@x.io", channel="email")[
            "allowed"
        ]
        is False
    )
