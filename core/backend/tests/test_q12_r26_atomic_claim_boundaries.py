...
def test_q12_r31_atomic_claim_writes_used_at_column(db_session: Session) -> None:
...
def test_q12_r31_manual_atomic_update_returns_zero_on_used_row(
    db_session: Session,
) -> None:
...
def test_q12_r31_manual_update_without_predicate_DOES_overwrite(
    db_session: Session,
) -> None:
...
def test_q12_r31_revoke_family_revokes_exactly_chain_length(
    db_session: Session,
) -> None:
...
def test_q12_r31_replay_after_success_raises_specific_oauth_error(
    db_session: Session,
) -> None:
...
def test_q12_r31_mid_chain_replay_revokes_root_to_tail(
    db_session: Session,
) -> None:
