...
The pre-existing tests in `test_admin_analytics.py` cover
...
def test_response_shape_contract(client, monkeypatch, _seed_three_sources):
...
def test_count_matches_entries_length(client, monkeypatch, _seed_three_sources):
    token = _login(client, monkeypatch)
    body = client.get(
        "/v1/admin/audit/recent",
        headers={"Authorization": f"Bearer {token}"},
...
def test_descending_sort_order(client, monkeypatch, _seed_three_sources):
    token = _login(client, monkeypatch)
    body = client.get(
        "/v1/admin/audit/recent",
        headers={"Authorization": f"Bearer {token}"},
...
def test_source_filter_customer_only(client, monkeypatch, _seed_three_sources):
    token = _login(client, monkeypatch)
    body = client.get(
...
def test_source_filter_webhook_only(client, monkeypatch, _seed_three_sources):
    token = _login(client, monkeypatch)
    body = client.get(
...
def test_limit_slicing_two(client, monkeypatch, _seed_three_sources):
    token = _login(client, monkeypatch)
    body = client.get(
...
def test_limit_one_boundary(client, monkeypatch, _seed_three_sources):
    token = _login(client, monkeypatch)
    body = client.get(
...
def test_limit_zero_returns_empty(client, monkeypatch, _seed_three_sources):
...
def test_huge_limit_is_safe(client, monkeypatch, _seed_three_sources):
...
def test_per_source_keys_present(client, monkeypatch, _seed_three_sources):
    token = _login(client, monkeypatch)
...
def test_audit_actor_present_all_sources(client, monkeypatch, _seed_three_sources):
...
def test_audit_customer_resource_and_webhook_detail_passthrough(client, monkeypatch):
