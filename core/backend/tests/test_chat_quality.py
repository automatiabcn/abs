...
def test_pipeline_router_keywords():
    ...
    assert detect_pipeline("Bunu Turkceden Ingilizceye cevir") == "qual_translate"
    assert detect_pipeline("neden bu strateji daha iyi, karsilastir") == "qual_analysis"
...
def test_build_citation_prompt_block_injects_numbered_chunks():
    ...
    rendered = build_citation_prompt_block(citations, user_message="What is bar?")
...
def test_cost_helper_unknown_provider_is_free():
    out = estimate_call_cost_usd(provider=None, tokens_in=100, tokens_out=400)
    assert out["free"] is True
    assert out["usd"] == 0.0


def test_cost_helper_priced_provider_returns_shape():
    out = estimate_call_cost_usd(provider="groq", tokens_in=100, tokens_out=400)
...
def test_chat_completion_meta_includes_pipeline_cost_and_chain(auth_client):
    r = auth_client.post(
        "/v1/chat/completions",
        json={
            "messages": [{"role": "user", "content": "Bir Python fonksiyonu yaz"}],
            "stream": True,
        },
    )
    ...
    assert "fallback_chain" in meta and isinstance(meta["fallback_chain"], list)
...
def test_chat_explicit_pipeline_override_skips_auto_detect(auth_client):
    r = auth_client.post(
        "/v1/chat/completions",
        json={
            "messages": [{"role": "user", "content": "Python kod yaz"}],
        ...
        return [
            {
                ...
                "score": 0.72,
                "payload": {
                    "text": "Kira her ayin 5'inde odenir.",
                    "filename": "kira.docx",
                },
            }
        ]
