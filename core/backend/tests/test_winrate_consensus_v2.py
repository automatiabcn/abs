...
def test_judge_one_dispatches_per_provider(consensus_mod, monkeypatch):
    ...
    assert consensus_mod.judge_one(
        "task", ["t1"], "ans-a", "ans-b",
        provider="groq", model="llama-3.3-70b-versatile", keys=keys) == "A"
    assert consensus_mod.judge_one(
        "task", ["t1"], "ans-a", "ans-b",
        provider="anthropic", model="claude-sonnet-4-5", keys=keys) == "B"
    assert consensus_mod.judge_one(
        "task", ["t1"], "ans-a", "ans-b",
        provider="gemini", model="gemini-2.5-pro", keys=keys) == "TIE"
    assert consensus_mod.judge_one(
        "task", ["t1"], "ans-a", "ans-b",
        provider="cohere", model="command-r-plus-08-2024", keys=keys) == "A"
...
def test_judge_one_missing_key_returns_error_not_tie(consensus_mod):
    ...
    assert consensus_mod.judge_one(
        "t", [], "x", "y", provider="anthropic",
        model="claude-sonnet-4-5", keys=keys) == "ERROR"
    assert consensus_mod.judge_one(
        "t", [], "x", "y", provider="gemini",
        model="gemini-2.5-pro", keys=keys) == "ERROR"
    assert consensus_mod.judge_one(
        "t", [], "x", "y", provider="cohere",
        model="command-r-plus-08-2024", keys=keys) == "ERROR"


def test_judge_one_rate_limit_after_retry_returns_error(
        consensus_mod, monkeypatch):
...
def test_rate_limit_retry_eventually_succeeds(base_mod, monkeypatch):
    ...
    result = base_mod._post_with_retry(
        "u", {}, {}, provider="groq", base_sleep=0.01, max_sleep=0.02,
        max_retries=4)
...
def test_rate_limit_retry_gives_up_after_max(base_mod, monkeypatch):
    ...
    with pytest.raises(base_mod.RateLimitError):
        base_mod._post_with_retry(
            "u", {}, {}, provider="cohere", base_sleep=0.001, max_sleep=0.002,
            max_retries=2)
...
def test_anthropic_throttle_blocks_at_cap(base_mod, monkeypatch):
    ...
    monkeypatch.setattr(base_mod.time, "time",
                        lambda: fake_now[0])
