...
def test_add_and_get_role() -> None:
    pm.add_member(
        tenant_slug="t1",
        project_slug="p1",
        user_subject="a@x.com",
        role=pm.ROLE_EDITOR,
    )
    assert (
        pm.get_role(tenant_slug="t1", project_slug="p1", user_subject="a@x.com")
        == "editor"
    )
...
def test_remove_member_soft_revokes() -> None:
    ...
    assert (
        pm.remove_member(tenant_slug="t4", project_slug="p", user_subject="a@x.com")
        is True
    )
    assert (
        pm.get_role(tenant_slug="t4", project_slug="p", user_subject="a@x.com") is None
    )
...
def test_readd_after_revoke_reactivates() -> None:
    ...
    assert (
        pm.get_role(tenant_slug="t5", project_slug="p", user_subject="a@x.com")
        == "editor"
    )
...
def test_tenant_isolation_membership() -> None:
    ...
    assert (
        pm.get_role(tenant_slug="tB", project_slug="shared", user_subject="a@x.com")
        is None
    )
