...
def test_crm_health_complete_is_100_no_fixes():
    t = "t_crm_full"
    with Session(get_engine()) as db:
        c = _seed_company(db, t, name="Tam A.Ş.", domain="tam.com", sector="İnşaat", vkn="1234567890")
        db.add(Contact(tenant_slug=t, company_id=c.id, name="Ali", email="ali@tam.com",
                       role="decision_maker", consent_status="opt-in"))
        db.add(Lead(tenant_slug=t, company_id=c.id, source="erp", score=0.8, intent="high"))
        db.commit()
...
def test_crm_health_counts_missing_fields():
    ...
    with Session(get_engine()) as db:
        ...
        db.add(Contact(tenant_slug=t, company_id=c.id, name="Veli", email="veli@eksik.com"))
...
def test_campaign_sums_attributed_and_picks_top_channel():
    ...
    with Session(get_engine()) as db:
        ...
        db.add(Opportunity(tenant_slug=t, company_id=c.id, name="D1", amount=100000,
                           currency="TRY", campaign="Meta Ads"))
        db.add(Opportunity(tenant_slug=t, company_id=c.id, name="D2", amount=300000,
                           currency="TRY", campaign="Meta Ads"))
        db.add(Opportunity(tenant_slug=t, company_id=c.id, name="D3", amount=50000,
                           currency="TRY", campaign="Google Ads"))
        db.commit()
...
def test_campaign_falls_back_to_total_pipeline_when_unattributed():
    ...
    with Session(get_engine()) as db:
        ...
        db.add(Opportunity(tenant_slug=t, company_id=c.id, name="P1", amount=70000, currency="USD"))
        db.commit()
        out = compute_campaign(db, t)
...
def test_buying_signals_fused_from_records():
    ...
    with Session(get_engine()) as db:
        ...
        db.add(Lead(tenant_slug=t, company_id=warm.id, score=0.5, intent="medium", status="engaged"))
        db.add(Opportunity(tenant_slug=t, company_id=hot.id, name="Fresh", amount=120000,
                           currency="TRY", created_at=datetime.now(timezone.utc)))
        db.commit()
...
def test_aeo_neutral_fallback_when_no_run():
    assert aeo_from_payload(None) == {
        "visibility_pct": None, "down_categories": 0, "categories_total": 8}


def test_aeo_passthrough_from_payload():
    out = aeo_from_payload({"visibility_pct": 41, "down_categories": 2, "categories_total": 8})
