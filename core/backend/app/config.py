...
class Settings(BaseSettings):
    ...
    # Next to the artefacts it describes, not on the marketing site.
    #
    # This pointed at app.automatiabcn.com/releases/manifest.json, which has
    # always returned the 404 page — so every customer's six-hourly update
    # check has failed silently since the feature was written. The archive and
    # the images live on the download host, and a manifest published anywhere
    # else is a second place to be right about the same release.
    #
    # Generated by `build_server_archive.sh --publish`, after the gate that
    # refuses a version whose images predate the code, so it cannot advertise a
    # release that is not really current.
    update_manifest_url: str = "https://dl.168-119-104-24.nip.io/manifest.json"
