...
> Generated: 2026-07-14T01:18:54+00:00 · duration: 0.26s · `7/7` rounds green
...
|---|---|---|---|---|---|
| `—` | anthropic → groq → cerebras → gemini → cloudflare → cohere | `anthropic` | `anthropic` | 137.9 | ✅ |
| `anthropic` | anthropic → groq → cerebras → gemini → cloudflare → cohere | `groq` | `groq` | 0.94 | ✅ |
| `groq` | anthropic → groq → cerebras → gemini → cloudflare → cohere | `anthropic` | `anthropic` | 0.57 | ✅ |
| `cerebras` | anthropic → groq → cerebras → gemini → cloudflare → cohere | `anthropic` | `anthropic` | 0.55 | ✅ |
| `gemini` | anthropic → groq → cerebras → gemini → cloudflare → cohere | `anthropic` | `anthropic` | 0.53 | ✅ |
| `cloudflare` | anthropic → groq → cerebras → gemini → cloudflare → cohere | `anthropic` | `anthropic` | 0.5 | ✅ |
| `cohere` | anthropic → groq → cerebras → gemini → cloudflare → cohere | `anthropic` | `anthropic` | 0.5 | ✅ |
