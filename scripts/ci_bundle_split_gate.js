#!/usr/bin/env node
// CI gate: exit non-zero if any code-split decision
...
if (reverts.length === 0) {
  console.log(`bundle-split gate: 0 REVERT verdicts across ${rows.length} rows. PASS`);
  process.exit(0);
}

console.error("bundle-split gate: REVERT verdict(s) detected — code-split harms LCP:");
